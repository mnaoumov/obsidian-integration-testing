 ...
 * @file
 *
 * Custom API documentation generator for obsidian-integration-testing.
 *
 * Follows the `src/index.ts` barrel to the modules it re-exports, extracts every publicly re-exported
 * declaration from them via ts-morph, and generates Starlight-compatible MDX pages plus a sidebar
 * JSON. The output tree mirrors the source module structure (a type's namespace is its source path
 * relative to `src`); every documented name is importable from the package root.
 *
...
async function main(): Promise<void> {
  ...
  // Load via the repo tsconfig so inferred types resolve.
  ...
  /*
   * Inheritance is resolved BEFORE the public filter, so a public class still inherits the members of
   * an internal base class it extends. Only then are the internal declarations themselves dropped.
   */
  resolveInheritedMembers(types);

  const publicNames = findPublicApiNames(srcDirectory);
  for (const [key, info] of types) {
    if (!publicNames.has(info.name)) {
      types.delete(key);
    }
  }
