...
const SITE_BASE_URL = 'https://mnaoumov.dev/obsidian-integration-testing/';
const HTTP_OK = 200;
...
describe('link-check parsing helpers', () => {
  ...
  it('derives the served URL from an index page and a leaf page', () => {
    expect(getPageUrl('index.html', SITE_BASE_URL)).toBe('https://mnaoumov.dev/obsidian-integration-testing/');
    expect(getPageUrl('api/foo/index.html', SITE_BASE_URL)).toBe('https://mnaoumov.dev/obsidian-integration-testing/api/foo/');
    expect(getPageUrl(String.raw`guides\bar.html`, SITE_BASE_URL)).toBe('https://mnaoumov.dev/obsidian-integration-testing/guides/bar.html');
  });
});
...
describe('isInternalDocUrl / getOutputRelativePath', () => {
  it('treats same-origin under-base URLs as internal and decodes their output path', () => {
    const url = new URL('https://mnaoumov.dev/obsidian-integration-testing/api/abort%20controller/');
...
describe('checkLinks (internal, offline)', () => {
  const pages: DocumentationPage[] = [{
    html: [
      '<a href="/obsidian-integration-testing/api/bravo/">ok page</a>',
      '<a href="../bravo/#known">ok fragment</a>',
      '<a href="/obsidian-integration-testing/api/bravo/#ghost">bad fragment</a>',
      '<a href="/obsidian-integration-testing/api/missing/">bad page</a>',
  ...
  it('reports missing pages and missing fragments while ignoring external and non-http links', () => {
    expect(collectBrokenLinks(pages, fileSystem, SITE_BASE_URL)).toEqual([
      { pageUrl: 'https://mnaoumov.dev/obsidian-integration-testing/api/alpha/', reason: 'missing-fragment', targetUrl: 'https://mnaoumov.dev/obsidian-integration-testing/api/bravo/#ghost' },
      { pageUrl: 'https://mnaoumov.dev/obsidian-integration-testing/api/alpha/', reason: 'missing-page', targetUrl: 'https://mnaoumov.dev/obsidian-integration-testing/api/missing/' }
    ]);
  });
});
...
describe('collectExternalTargets (dedup)', () => {
  it('deduplicates external URLs across pages and records one referencing page', () => {
    const pages: DocumentationPage[] = [
      ...
      { html: '<a href="https://developer.mozilla.org/x">dup</a><a href="/obsidian-integration-testing/internal/">skip</a><a href="mailto:x@y.z">skip</a>', relativePath: 'p2/index.html' }
    ];

    expect(collectExternalTargets(pages, SITE_BASE_URL)).toEqual(new Map([
      ['https://developer.mozilla.org/x', 'https://mnaoumov.dev/obsidian-integration-testing/p1/'],
      ['https://github.com/y', 'https://mnaoumov.dev/obsidian-integration-testing/p1/']
    ]));
  });
});
...
describe('checkExternalTargets (network, concurrency-bounded)', () => {
  it('probes each unique URL once and reports non-200 and network failures', async () => {
    const targets = new Map([
      ['https://down.example/', 'https://mnaoumov.dev/obsidian-integration-testing/p/'],
      ['https://missing.example/', 'https://mnaoumov.dev/obsidian-integration-testing/p/'],
      ['https://ok.example/', 'https://mnaoumov.dev/obsidian-integration-testing/p/']
    ]);
    ...
    expect(brokenLinks).toEqual([
      { httpStatus: NETWORK_FAILURE_STATUS, pageUrl: 'https://mnaoumov.dev/obsidian-integration-testing/p/', reason: 'external-error', targetUrl: 'https://down.example/' },
      { httpStatus: HTTP_NOT_FOUND, pageUrl: 'https://mnaoumov.dev/obsidian-integration-testing/p/', reason: 'external-error', targetUrl: 'https://missing.example/' }
    ]);
  });

  it('never exceeds the configured concurrency', async () => {
    const targets = new Map(Array.from({ length: TARGET_COUNT }, (_unused, index) => [`https://host-${String(index)}.example/`, 'https://mnaoumov.dev/obsidian-integration-testing/p/']));
    let inFlight = 0;
    let maxInFlight = 0;
