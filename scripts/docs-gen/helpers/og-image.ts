...
const BRAND_TEXT = 'obsidian-integration-testing';
 ...
 *
 * The Inter TTFs are vendored under `scripts/docs-gen/assets/fonts`; if they are ever missing this
 * falls back to any other `.ttf`/`.otf` in `fontsDirectory`, and returns `null` when none are
 * available, so the caller can skip OG generation instead of crashing the build.
 */
...
function buildOgImageMarkup(params: OgImageParams, logoDataUri: null | string): Record<string, unknown> {
  ...
  // Footer with text branding next to the rasterized favicon
  const footer = {
    props: {
      children: [
