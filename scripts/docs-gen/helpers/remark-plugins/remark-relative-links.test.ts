...
describe('remarkRelativeLinks', () => {
  it('rewrites based internal links relative to the current content page', () => {
    const tree: Root = {
      children: [{
        ...
        type: 'link',
        url: '/obsidian-integration-testing/api/alpha/Alpha/#member'
      }],
      type: 'root'
    };
    const transformer = remarkRelativeLinks('/obsidian-integration-testing')();
  ...
  it('rewrites a self-link to the current directory instead of emitting a 404-ing double slash', () => {
    const tree: Root = {
      children: [
        { children: [{ type: 'text', value: 'this' }], type: 'link', url: '/obsidian-integration-testing/api/path/join/' },
        { children: [{ type: 'text', value: 'section' }], type: 'link', url: '/obsidian-integration-testing/api/path/join/#overloads' }
      ],
      type: 'root'
    };
    const transformer = remarkRelativeLinks('/obsidian-integration-testing')();
