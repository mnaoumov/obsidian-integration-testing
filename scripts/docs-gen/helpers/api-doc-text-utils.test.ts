...
describe('api-doc text utilities', () => {
  ...
  it('renders API display and import metadata', () => {
    ...
    expect(getImportStatement(typeInfo)).toBe('import type { Alpha } from \'obsidian-integration-testing\';');
    expect(getImportStatement(createTypeInfo({ kind: 'function', name: 'bravo', namespace: 'bravo' }))).toBe('import { bravo } from \'obsidian-integration-testing\';');
  });
