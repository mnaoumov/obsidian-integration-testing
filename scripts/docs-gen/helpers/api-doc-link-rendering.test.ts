...
describe('api-doc link rendering', () => {
  ...
  it('resolves same-namespace types but leaves ambiguous names unlinked', () => {
    ...
    expect(renderTypeWithLinks('Alpha | Promise<string>', types, undefined, 'local')).toContain('[Alpha](/obsidian-integration-testing/api/local/Alpha/)');
  ...
  it('links only generated member pages and renders safe MDX prose', () => {
    ...
    expect(resolveLinks('{@link PluginNoticeComponent.showNotice}', types, 'alpha')).toBe('[PluginNoticeComponent.showNotice](/obsidian-integration-testing/api/alpha/PluginNoticeComponent/showNotice/)');
    expect(renderMdxProse('Use <Alpha> and {@link PluginNoticeComponent}.', types, 'alpha')).toBe(String.raw`Use \<Alpha> and [PluginNoticeComponent](/obsidian-integration-testing/api/alpha/PluginNoticeComponent/).`);
  });
