 ...
 *
 * Every documented name comes from the package root — the namespace groups the reference by source
 * module, but it is not an import subpath, so `src/eval-in-obsidian.ts` still reads
...
export function getImportStatement(info: TypeInfo): string | undefined {
  ...
  return `${importKeyword} { ${info.name} } from '${PACKAGE_NAME}';`;
}
