 ...
 *
 * The model map is keyed by a qualified `${namespace}#${name}` identity, because the library can
 * export the same simple name from different modules. Resolution is best-effort:
...
export function resolveTsUtilityUrl(name: string): string | undefined {
  ...
  if (hash) {
    ...
    return ['Capitalize', 'Lowercase', 'Uncapitalize', 'Uppercase'].includes(name) // Cspell:disable-line
