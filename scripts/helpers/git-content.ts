 ...
 *
 * Two reasons, and either alone would be enough. Both are properties of `scripts/helpers/exec.ts` as it
 * stands (re-read 2026-09-19), not of how it is called:
 *
 * - **It decodes stdout as UTF-8**, accumulating `data.toString('utf-8')` into a string. A blob that is not
 *   valid UTF-8 does not survive that round trip. This repo has such blobs tracked - the two Inter fonts
 *   under `scripts/docs-gen/assets/fonts` - and a primitive whose byte-exactness depends on which files a
 *   caller happens to hand it is the wrong thing to build a byte-identity gate on.
 * - **It strips one trailing newline**, in its `stdout` `end` handler. Every vendored rule source ends with
 *   one, so every one of them would read as differing from upstream by its last line.
 *
...
export function readIndexContent(root: string, repoRelativePath: string): Promise<Buffer | null> {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (exitCode) => {
       ...
       * vendored-rules gate finds its files by walking the tree, so it can legitimately reach one and reads
       * it from disk.
       */
