 ...
 *
 * Only the parameters `lintMarkdownContent` passes are declared. The full parameter object also carries
 * `argv`, `allowStdin`, `fileContents`, `fs`, `noImport`, `optionsDefault` and `optionsOverride`, none of
 * which a caller linting one in-memory document against this repository's own configuration has any use for
 * -- and declaring an option nobody passes is a claim about a shape nothing here ever checks.
 */
