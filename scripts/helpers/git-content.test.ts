 ...
 * mangled the vendored Inter fonts had the roster ever reached them, and the second would have reported all
 * eleven vendored rule sources as differing from upstream by their last line.
 */
...
describe('readIndexContent', () => {
  ...
  it('reads a tracked text file as the bytes a commit would write', async () => {
    ...
    expect(parsed.name).toBe('obsidian-integration-testing');
  });
