import type { FileSystemAdapter } from 'obsidian';

import {
  afterAll,
  beforeAll,
  describe,
  expect,
  expectTypeOf,
  it
} from 'vitest';

import { ContextId } from './context-id.ts';
import { evalInObsidian } from './eval-in-obsidian.ts';
import { TemporaryVault } from './temporary-vault.ts';

interface AbArguments {
  a: number;
  b: number;
}

interface CreateNoteProbe {
  content: string;
  path: string;
}

interface CreateNoteRepairProbe {
  content: string;
  modifyCallCount: number;
}

const temporaryVault = new TemporaryVault();
let vaultPath: string;

const REGISTRATION_TIMEOUT_IN_MILLISECONDS = 60_000;

beforeAll(async () => {
  await temporaryVault.register();
  vaultPath = temporaryVault.path;
}, REGISTRATION_TIMEOUT_IN_MILLISECONDS);

afterAll(async () => {
  await temporaryVault.dispose();
});

describe('eval-in-obsidian integration', () => {
  it('should access the vault base path via app.vault.adapter.getBasePath()', async () => {
    const basePath = await evalInObsidian({
      callback({ app }): string {
        return (app.vault.adapter as FileSystemAdapter).getBasePath();
      },
      vaultPath: temporaryVault.path
    });
    expect(basePath).toBe(vaultPath);
  });

  it('should evaluate an anonymous arrow function', async () => {
    const result = await evalInObsidian({
      callback: ({ a, b }): number => a + b,
      input: {
        a: 2,
        b: 3
      },
      vaultPath
    });
    expect(result).toBe(5);
  });

  it('should evaluate an anonymous function expression', async () => {
    const result = await evalInObsidian({
      callback({ a, b }): number {
        return a * b;
      },
      input: {
        a: 4,
        b: 5
      },
      vaultPath
    });
    expect(result).toBe(20);
  });

  it('should evaluate a function declaration', async () => {
    function add({ a, b }: AbArguments): number {
      return a + b;
    }
    const result = await evalInObsidian({ callback: add, input: { a: 10, b: 20 }, vaultPath });
    expect(result).toBe(30);
  });

  it('should evaluate a shorthand method', async () => {
    const object = {
      add(this: void, { a, b }: AbArguments): number {
        return a + b;
      }
    };
    const result = await evalInObsidian({ callback: object.add, input: { a: 7, b: 8 }, vaultPath });
    expect(result).toBe(15);
  });

  it('should evaluate an async function', async () => {
    async function addAsync({ a, b }: AbArguments): Promise<number> {
      // eslint-disable-next-line unicorn/no-useless-promise-resolve-reject -- The `await` is the point: this fixture exists to prove a genuinely asynchronous function survives serialization and resolves inside Obsidian.
      return await Promise.resolve(a + b);
    }
    const result = await evalInObsidian({ callback: addAsync, input: { a: 100, b: 200 }, vaultPath });
    expect(result).toBe(300);
  });

  it('should evaluate an async shorthand method', async () => {
    const object = {
      async multiply(this: void, { a, b }: AbArguments): Promise<number> {
        // eslint-disable-next-line unicorn/no-useless-promise-resolve-reject -- The `await` is the point: this fixture exists to prove a genuinely asynchronous shorthand method survives serialization and resolves inside Obsidian.
        return await Promise.resolve(a * b);
      }
    };
    const result = await evalInObsidian({
      callback: object.multiply,
      input: {
        a: 6,
        b: 7
      },
      vaultPath
    });
    expect(result).toBe(42);
  });

  it('should pass string input', async () => {
    interface Input {
      parts: string[];
      separator: string;
    }
    const result = await evalInObsidian({ callback: concat, input: { parts: ['a', 'b', 'c'], separator: '-' }, vaultPath });
    expect(result).toBe('a-b-c');

    function concat({ parts, separator }: Input): string {
      return parts.join(separator);
    }
  });

  it('should pass input with a multi-line function declaration', async () => {
    interface Input {
      base: number;
      factor: number;
    }
    function compute({ base, factor }: Input): number {
      const doubled = base * 2;
      const result = doubled * factor;
      return result;
    }
    const result = await evalInObsidian({ callback: compute, input: { base: 5, factor: 3 }, vaultPath });
    expect(result).toBe(30);
  });

  it('should pass input with an async shorthand method', async () => {
    const object = {
      async compute(this: void, { a, b }: AbArguments): Promise<number> {
        const sum = await Promise.resolve(a + b);
        return sum * 2;
      }
    };
    const result = await evalInObsidian({ callback: object.compute, input: { a: 3, b: 4 }, vaultPath });
    expect(result).toBe(14);
  });

  it('should preserve newlines in template literals', async () => {
    interface Input {
      name: string;
    }
    function withTemplate({ name }: Input): string {
      const text = `hello
world
${name}`;
      return text;
    }
    const result = await evalInObsidian({ callback: withTemplate, input: { name: 'test' }, vaultPath });
    expect(result).toBe('hello\nworld\ntest');
  });

  it('should return void for a void function', async () => {
    expectTypeOf(
      // eslint-disable-next-line @typescript-eslint/no-confusing-void-expression -- Testing void function.
      await evalInObsidian({
        callback() {
          // Cannot use `noop()` here because `evalInObsidian()` does not accept functions with external imports.
        },
        vaultPath
      })
    ).toBeVoid();
  });

  it('should return void for an async void function', async () => {
    expectTypeOf(
      // eslint-disable-next-line @typescript-eslint/no-confusing-void-expression -- Testing void function.
      await evalInObsidian({
        async callback(): Promise<void> {
          await Promise.resolve();
        },
        vaultPath
      })
    ).toBeVoid();
  });

  it('should provide the obsidian module via obsidianModule', async () => {
    const result = await evalInObsidian({
      callback({ obsidianModule }): string {
        return obsidianModule.stringifyYaml({ a: 1 });
      },
      vaultPath
    });
    expect(result).toBe('a: 1\n');
  });

  it('should access the public Obsidian API via app', async () => {
    const isConfigDirectoryHidden = await evalInObsidian({
      callback({ app }): boolean {
        return app.vault.configDir.startsWith('.');
      },
      vaultPath
    });
    expect(isConfigDirectoryHidden).toBe(true);
  });

  it('should access the internal Obsidian API via app', async () => {
    const doesTitleIncludeObsidian = await evalInObsidian({
      callback({ app }): boolean {
        return app.title.includes('Obsidian');
      },
      vaultPath
    });
    expect(doesTitleIncludeObsidian).toBe(true);
  });

  it('should pass function input and execute them in the Obsidian context', async () => {
    const result = await evalInObsidian({
      callback({ transform, value }): number {
        return transform(value);
      },
      input: {
        transform(this: void, x: number): number {
          return x * 2;
        },
        value: 5
      },
      vaultPath
    });
    expect(result).toBe(10);
  });

  describe('vault targeting', () => {
    it('should work when a different vault is the active one (1b)', async () => {
      // The main Obsidian vault is open, but we target the temp vault via vaultPath.
      const result = await evalInObsidian({
        callback({ app }): string {
          return (app.vault.adapter as FileSystemAdapter).getBasePath();
        },
        vaultPath
      });
      expect(result).toBe(vaultPath);
    });

    it('should work when the target vault is already open (1c)', async () => {
      // The temp vault is already open from registration. Calling eval again should work.
      const first = await evalInObsidian({
        callback(): number {
          return 1;
        },
        vaultPath
      });
      const second = await evalInObsidian({
        callback(): number {
          return 2;
        },
        vaultPath
      });
      expect(first).toBe(1);
      expect(second).toBe(2);
    });

    it('should throw when targeting an unregistered vault path', async () => {
      await expect(evalInObsidian({
        callback(): number {
          return 1;
        },
        vaultPath: String.raw`C:\nonexistent\vault\path`
      })).rejects.toThrow('Vault path does not exist');
    });
  });

  describe('waitUntil', () => {
    it('should resolve once a synchronous predicate becomes truthy', async () => {
      const result = await evalInObsidian({
        async callback({ lib: { waitUntil } }): Promise<number> {
          const FLIP_AFTER_IN_MILLISECONDS = 200;
          const startTime = Date.now();
          let callCount = 0;
          await waitUntil({
            predicate(): boolean {
              callCount++;
              return Date.now() - startTime >= FLIP_AFTER_IN_MILLISECONDS;
            }
          });
          return callCount;
        },
        vaultPath
      });
      // Polled at least twice: false at least once, then truthy.
      expect(result).toBeGreaterThanOrEqual(2);
    });

    it('should await an asynchronous predicate', async () => {
      const hasAsyncPredicateSettled = await evalInObsidian({
        async callback({ lib: { waitUntil } }): Promise<boolean> {
          const FLIP_AFTER_IN_MILLISECONDS = 150;
          const startTime = Date.now();
          await waitUntil({
            async predicate(): Promise<boolean> {
              await Promise.resolve();
              return Date.now() - startTime >= FLIP_AFTER_IN_MILLISECONDS;
            }
          });
          return true;
        },
        vaultPath
      });
      expect(hasAsyncPredicateSettled).toBe(true);
    });

    it('should reject with the message when the predicate never becomes truthy', async () => {
      await expect(evalInObsidian({
        async callback({ lib: { waitUntil } }): Promise<void> {
          const TIMEOUT_IN_MILLISECONDS = 100;
          await waitUntil({
            intervalInMilliseconds: 10,
            message: 'never happened',
            predicate: (): boolean => false,
            timeoutInMilliseconds: TIMEOUT_IN_MILLISECONDS
          });
        },
        vaultPath
      })).rejects.toThrow('waitUntil timed out after 100 milliseconds: never happened');
    });

    it('should apply the default timeout when none is provided', async () => {
      const DEFAULT_TIMEOUT_IN_MILLISECONDS = 5000;
      const result = await evalInObsidian({
        async callback({ lib: { waitUntil } }): Promise<number> {
          const startTime = Date.now();
          try {
            await waitUntil({ predicate: (): boolean => false });
            return -1;
          } catch {
            return Date.now() - startTime;
          }
        },
        vaultPath
      });
      // The default timeout (5000 ms) fired: elapsed is close to it, not the ~50 ms interval.
      const LOWER_BOUND_IN_MILLISECONDS = 4000;
      const UPPER_BOUND_IN_MILLISECONDS = 8000;
      expect(result).toBeGreaterThanOrEqual(LOWER_BOUND_IN_MILLISECONDS);
      expect(result).toBeLessThan(UPPER_BOUND_IN_MILLISECONDS);
      expect(DEFAULT_TIMEOUT_IN_MILLISECONDS).toBeGreaterThan(LOWER_BOUND_IN_MILLISECONDS);
    });

    it('should poll at the fast default interval when none is provided', async () => {
      const result = await evalInObsidian({
        async callback({ lib: { waitUntil } }): Promise<number> {
          const FLIP_AFTER_IN_MILLISECONDS = 200;
          const startTime = Date.now();
          await waitUntil({
            predicate: (): boolean => Date.now() - startTime >= FLIP_AFTER_IN_MILLISECONDS
          });
          return Date.now() - startTime;
        },
        vaultPath
      });
      // With the default 50 ms interval, it resolves shortly after the 200 ms flip.
      // Well below the default 5000 ms timeout, proving the default interval is small.
      const UPPER_BOUND_IN_MILLISECONDS = 1000;
      expect(result).toBeLessThan(UPPER_BOUND_IN_MILLISECONDS);
    });
  });

  describe('createNote', () => {
    it('should create a note whose content is readable back', async () => {
      const result = await evalInObsidian({
        async callback({ app, lib: { createNote } }): Promise<CreateNoteProbe> {
          const path = 'create-note-basic.md';
          const content = 'created through the harness helper';
          const file = await createNote({ content, path });
          return {
            content: await app.vault.read(file),
            path: file.path
          };
        },
        vaultPath
      });

      expect(result).toEqual({
        content: 'created through the harness helper',
        path: 'create-note-basic.md'
      });
    });

    // The Android transport loses ~0.9% of `vault.create` writes: the file lands 0 bytes on disk while
    // `TFile.stat` reports the full size. It cannot be provoked on desktop, so the repair path is driven
    // By patching `vault.read` to report the lost-write symptom once — the same thing the helper sees.
    it('should rewrite a note whose content did not land, then confirm it', async () => {
      const result = await evalInObsidian({
        async callback({ app, lib: { createNote } }): Promise<CreateNoteRepairProbe> {
          const path = 'create-note-repair.md';
          const content = 'content that is lost on the first read';

          const vault = app.vault;
          const originalRead = vault.read.bind(vault);
          const originalModify = vault.modify.bind(vault);
          let modifyCallCount = 0;
          let hasReportedLostWrite = false;

          vault.read = async (file): Promise<string> => {
            if (!hasReportedLostWrite && file.path === path) {
              hasReportedLostWrite = true;
              return '';
            }
            return originalRead(file);
          };
          vault.modify = async (file, data): Promise<void> => {
            modifyCallCount++;
            await originalModify(file, data);
          };

          try {
            const file = await createNote({ content, path });
            return { content: await originalRead(file), modifyCallCount };
          } finally {
            vault.read = originalRead;
            vault.modify = originalModify;
          }
        },
        vaultPath
      });

      expect(result).toEqual({
        content: 'content that is lost on the first read',
        modifyCallCount: 1
      });
    });

    it('should throw naming the path when the content never lands', async () => {
      await expect(evalInObsidian({
        async callback({ app, lib: { createNote } }): Promise<void> {
          const path = 'create-note-hopeless.md';
          const vault = app.vault;
          const originalRead = vault.read.bind(vault);
          vault.read = async (file): Promise<string> => file.path === path ? '' : originalRead(file);

          try {
            await createNote({ content: 'never lands', path });
          } finally {
            vault.read = originalRead;
          }
        },
        vaultPath
      })).rejects.toThrow('createNote could not write "create-note-hopeless.md": expected 11 characters, the vault holds 0 after 3 repair attempts.');
    });
  });

  describe('context', () => {
    it('should persist values across calls and dispose cleanly', async () => {
      interface Context {
        storedValue: number;
      }
      const contextId = new ContextId<Context>();

      await evalInObsidian({
        callback({ context }): void {
          context.storedValue = 123;
        },
        contextId,
        vaultPath
      });

      const retrieved = await evalInObsidian({
        callback({ context: { storedValue } }): number {
          return storedValue;
        },
        contextId,
        vaultPath
      });
      expect(retrieved).toBe(123);

      await contextId.dispose(vaultPath);

      const isStoredValueCleared = await evalInObsidian({
        callback({ context }): boolean {
          return (context as Partial<typeof context>).storedValue === undefined;
        },
        contextId,
        vaultPath
      });
      expect(isStoredValueCleared).toBe(true);
    });
  });
});
