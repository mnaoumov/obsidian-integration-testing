 ...
 * @file
 *
 * Jest **plugin-less** global teardown entry point.
 *
 * Jest requires `globalTeardown` to be a separate module whose default export is
 * the teardown function. It re-exports the teardown from the plugin-less setup
 * module so both share the same setup state within the Jest parent process.
 */
...
export { teardown as default } from './global-setup-no-plugin.ts';
