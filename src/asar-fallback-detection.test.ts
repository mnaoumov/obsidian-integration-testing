...
describe('resolveAsarFallback', () => {
  it('is `unknown` when no asar version was requested', () => {
    const verdict = resolveAsarFallback({ requestedVersion: undefined, runningApiVersion: '1.1.9' });
  ...
  it('is `unknown` when the running version could not be read', () => {
    const verdict = resolveAsarFallback({ requestedVersion: '1.13.0', runningApiVersion: undefined });
  ...
  it('is `unknown` when both versions are absent', () => {
    const verdict = resolveAsarFallback({ requestedVersion: undefined, runningApiVersion: undefined });
  ...
  it('is `match` when the running version equals the requested version', () => {
    const verdict = resolveAsarFallback({ requestedVersion: '1.13.1', runningApiVersion: '1.13.1' });
  ...
  it('is `fallback` when the running version is older than the requested version (silent revert)', () => {
    const verdict = resolveAsarFallback({ requestedVersion: '1.13.0', runningApiVersion: '1.1.9' });
  ...
  it('is `fallback` for any mismatch, including a newer running version', () => {
    const verdict = resolveAsarFallback({ requestedVersion: '1.13.0', runningApiVersion: '1.13.1' });
    expect(verdict.tier).toBe('fallback');
    expect(verdict.requestedVersion).toBe('1.13.0');
